...
class HotelTenantSubscriberTest extends TestCase
{
    ...
    public function testRejectsWhenAnyOneArgumentBelongsToAnotherHotel(): void
    {
        ...
        $subscriber($this->event(
            [$this->fineFor(self::OWN_HOTEL), $this->bookingFor(self::OTHER_HOTEL)],
            'aureum_fines_edit',
        ));
    }
    ...
    public static function unownedRecordProvider(): array
    {
        return [
            ...
            'booking with no hotel set' => [static fn (): Booking => new Booking()],
        ];
    }
    ...
    private function bookingFor(int $hotelId): Booking
    {
        $booking = new Booking();
        $booking->setHotel($this->hotel($hotelId));
